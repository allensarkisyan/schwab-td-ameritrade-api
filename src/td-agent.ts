...
let tdApi: TDAmeritradeAPI | null = null;
let isRefreshingAcessToken = false;
let isInitialized = false;
let interval: NodeJS.Timeout | null = null;

const refreshTDAmeritradeAccessToken = async () => {
  if (isRefreshingAcessToken || !tdApi) { return; }

  try {
    const data = JSON.parse(fs.readFileSync('./td-token.json').toString('utf-8'));
    let { principals, userAccessToken, accessTokenExpires, refreshToken, refreshTokenExpires } = data;

    const now = Date.now();
    const fiveMinsFromNow = (now + ((60 * 1000) * 5));
    const accessTokenExpiresDt = new Date(accessTokenExpires).getTime();
    const isAccessTokenExpired = userAccessToken && accessTokenExpires && fiveMinsFromNow > accessTokenExpiresDt;

    console.log('ACCESS TOKEN EXPIRES', accessTokenExpires, isAccessTokenExpired);
    ...
    if (isAccessTokenExpired && !isRefreshingAcessToken) {
      ...
      fs.writeFileSync('./td-token.json', JSON.stringify({
...
export async function initializeTDAmeritradeAPIAgent({ userAccessToken, accessTokenExpires, refreshToken, refreshTokenExpires }) {
  if (isInitialized) {
    return;
  }
  ...
  const isAccessTokenExpired = userAccessToken && accessTokenExpires && fiveMinsFromNow > accessTokenExpiresDt;
  const isRefreshTokenExpired = refreshToken && refreshTokenExpires && fiveMinsFromNow > refreshTokenExpiresDt;
